...
function VidyoClientWebRTC(t) {
    ...
    function SendDeviceEnumerationResponse(devices) {
        ...
        if (devices.length > 0) {
            ...
            if (window.adapter.browserDetails.browser === "chrome" || window.adapter.browserDetails.browser === "firefox") {
        ...
        if (window.adapter.browserDetails.browser === "chrome") {
            ...
            navigator.mediaDevices.enumerateDevices().then(function(devices) {
                ...
                navigator.mediaDevices.getUserMedia(constraints).then(function(s) {
                    ...
                    LogErr("getUserMedia error in DeviceEnumeration: " + JSON.stringify(constraints) + " err:" + JSON.stringify(err));
                });
            }).catch(function(err) {
                LogErr("enumerateDevice error in DeviceEnumeration err:" + JSON.stringify(err));
            });
            return;
        }
            ...
            transport_.SendWebRTCMessage(offerMsg, function() {
                if (additionalOffers_[i]) {
                    ...
                    then(function() {
                        ...
                        LogErr("PeerConnection [" + (i+1) + "] setLocalDescription failed " + JSON.stringify(err));
                        console.log(err);
                    });
                }
            });
        }).catch(function(err) {
            LogErr("PeerConnection [" + (i+1) + "] CreateOffer failed " + JSON.stringify(err));
            console.log(err);
        });
            ...
            if (iceGatheringState !== "new") {
                ...
                additionalIceCandidates_[i].forEach(function(iceCandidate) {
                    ...
                    then(function() {
                        ...
                        LogErr("HandleIceCandidatein icegatheringstatchange [" + (i+1) + "] set failed - " + JSON.stringify(iceCandidate) + " " + err.stack + " " + JSON.stringify(err));
                        console.log(err);
                    });
                });
    ...
    function HandleIceCandidate(data) {
        ...
        if (data.streamId === 1 && peerConnection_ !== null) {
            ...
            then(function() {
                ...
                LogErr("HandleIceCandidate set failed - " + JSON.stringify(iceCandidate) + " " + err.stack + " " + JSON.stringify(err));
                console.log(err);
            });
            ...
            if (additionalPc_[data.streamId-1] !== null) {
                if (additionalPc_[data.streamId-1].iceGatheringState !== "new") {
                    ...
                    then(function() {
                        ...
                        LogErr("HandleIceCandidate [" + (data.streamId) + "] set failed - " + JSON.stringify(iceCandidate) + " " + err.stack + " " + JSON.stringify(err));
                        console.log(err);
                    });
                } else {
        ...
        var SetRemoteSdp = function() {
            if (additionalPc_[i]) {
                ...
                then(function() {
                    ...
                    LogErr("PeerConnection [" + (i+1) + "] setRemoteDescription failed " + JSON.stringify(err));
                    console.log(err);
                });
            } else {
...
function VidyoClientTransport(plugInObj, statusChangeHandler, callbackHandler, plugInDivId){
    ...
    this.UpdateViewOnDOM = function(uiEvent, parentDivId, arg1, arg2, arg3, arg4){
        ...
        if((uiEvent.indexOf("create") !== -1) || (uiEvent.indexOf("constructor") !== -1) || (uiEvent.indexOf("AssignView") !== -1)){
            if(parentDivId){
                var view = VCUtils.jQuery('#' + parentDivId);
        ...
        if (uiEvent === "constructor" || uiEvent === "create") {
            ...
            transport.GetWebRTCClient().showViewLabel(plugInDivId, arg1);
        }
...
function VidyoClientTransportWebRTC(plugInObj, statusChangeHandler, callbackHandler, plugInDivId, callback){
    ...
    var CheckAndSendMessage = function() {
        if (connectionState === "CONNECTING") {
            LogInfo("CheckAndSendMessage: Waiting for CONNECTED QLen=" + requestQueue.length);
            return;
        }
        if (requestPending >= 0) {
